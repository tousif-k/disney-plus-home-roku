...
' It also handles the extraction of reference IDs and item details from the JSON response.
...
function GetItems(refId as Object) as Object
    ...
    if data <> invalid and data.data <> invalid then
        ' this for loop is to get the items object and work around the different json structure
        for each key in data.data
            if type(data.data[key]) = "roAssociativeArray" and data.data[key].items <> invalid then
...
function GetTitle(title as Object) as String
    if title <> invalid
        for each key in title
            if type(title[key]) = "roAssociativeArray" and title[key].default.content <> invalid then
...
function GetThumbnailUrl(imageData as Object) as String
    if imageData <> invalid and imageData["1.78"] <> invalid
        for each key in imageData["1.78"]
            if type(imageData["1.78"][key]) = "roAssociativeArray" and imageData["1.78"][key].default.url <> invalid then
